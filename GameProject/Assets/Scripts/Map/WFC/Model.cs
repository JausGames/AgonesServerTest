...
abstract class Model
{
    ...
    void Observe(int node)
    {
        ...
        // Calculate the cumulative distribution based on weights
        double[] cumulativeDistribution = new double[T];
        ...
        for (int t = 0; t < T; t++)
        {
            ...
            cumulativeDistribution[t] = cumulativeWeight;
        }

        // Generate a random value between 0 and 1
        double randomValue = UnityEngine.Random.value;

        // Find the selected tile index
        for (int t = 0; t < T; t++)
        {
            if (cumulativeDistribution[t] >= randomValue)
            {
                candidates.Add(t);
            }
        }
        ...
        if (candidates.Count > 0)
        {
            ...
            // Ban all other tiles except the selected one
            for (int t = 0; t < T; t++)
            {
                if (t != selectedIndex)
                {
        ...
        {
            // Handle the case when there are no valid candidates (shouldn't happen)
            UnityEngine.Debug.LogError("No valid candidates found in Observe method.");
        }
    }
