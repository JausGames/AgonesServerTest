...
{
    List<byte[]> patterns;
    List<UnityEngine.Sprite> colors;

    public OverlappingModel(Tilemap tilemap, int N, int width, int height, bool periodicInput, bool periodic, int symmetry, bool ground, Heuristic heuristic)
        : base(width, height, N, periodic, heuristic)
    {
        //var (bitmap, SX, SY) = BitmapHelper.LoadBitmap($"samples/{name}.png");
        /*UnityEngine.BoundsInt bounds = tilemap.cellBounds;
        var bitmap = tilemap.GetTilesBlock(bounds);
        var SX = bounds.size.x;
        var SY = bounds.size.y;
        ...
        for (int x = 0; x < bounds.size.x; x++)
        {
            for (int y = 0; y < bounds.size.y; y++)
            {
                ...
                if (tile != null)
                {
                    UnityEngine.Debug.Log("x:" + x + " y:" + y + " tile:" + tile.name);
                }
                else
                {
                    UnityEngine.Debug.Log("x:" + x + " y:" + y + " tile: (null)");
                }
            }
        }

        TileBase[] sample = new TileBase[allTiles.Length];
        colors = new List<UnityEngine.Sprite>();
        for (int i = 0; i < sample.Length; i++)
        {
            UnityEngine.Sprite color = ((Tile)(bitmap[i])).sprite;
            int k = 0;
