...
public class BasicEnemy : Hitable
{
    ...
    {
        Debug.Log("BasicEnemy, UseWeapon");
        ((EnemySoundManager)soundManager).PlayPreAttack();
        controller.StopAgent();
        yield return new WaitForSeconds(.3f);
        if (fsm.currentState.type != StateType.Dead)
        {
            weapon.Use();
            ((EnemySoundManager)soundManager).PlayAttack();
            yield return new WaitForSeconds(.3f);
        ...
        switch (fsm.currentState.type)
        {
            case StateType.CheckForTarget:
                ...
                if (target != null)
                {
                    ...
                    controller.Destination = target.transform.position;
                    ((EnemySoundManager)soundManager).PlayDetection();
                }
                break;
            case StateType.MoveToTarget:
                // :(
                if (target == null) fsm.currentState.type = StateType.CheckForTarget;
                else if ((transform.position - target.transform.position).sqrMagnitude <= checkForHitRadius)
                {
